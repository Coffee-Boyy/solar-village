...
This README walks you through setting up a **three-node local cluster** that mirrors the production-grade decentralized infrastructure:

* **3× MySQL 8.4 Group Replication** (multi-primary)
* **3× Garage distributed object storage nodes**
* **3× Ushahidi containers + Traefik reverse proxies**
...
5. Bring up MySQL Group Replication.
...
```bash
ssh ubuntu@<IP_NODE_A> "docker exec mysql mysql -uroot -p'rootpass' -e \"SELECT MEMBER_HOST,MEMBER_STATE FROM performance_schema.replication_group_members;\""
```
...
Garage will be accessible at `http://<IP_NODE_A>:3900` (S3 API) and `http://<IP_NODE_A>:3903` (Admin API).

### Ushahidi API

The API is available at `http://<IP_NODE_A>:9080` (or any node's :9080).

You can obtain an OAuth token by using the default admin credentials to test the API:

```bash
curl --location 'http://<IP_NODE_A>:9080/oauth/token' \
...
--data-urlencode 'scope=posts country_codes media forms api tags savedsearches sets users stats layers config messages notifications webhooks contacts roles permissions csv tos dataproviders migrate apikeys'
```

Full API documentation for Ushahidi V5 is available here: https://documenter.getpostman.com/view/24695099/2s8YszQWLk
...
| Restart Node B     | `multipass restart node-b`   | GR rejoins automatically |
...
| DNS name not resolving       | Local host mapping needed     | `sudo nano /etc/hosts` → `<IP_NODE_A> ushahidi.local.test` |
